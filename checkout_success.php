<?php
// --- START LOGIKA CHECKOUT ---
if (session_status() === PHP_SESSION_NONE) {
  session_start();
}

require_once('includes/session.php');
require_once('includes/koneksi.php'); // koneksi database
...
$stmt = $conn->prepare("
    SELECT c.quantity, s.id, s.name, s.price, s.images
    FROM cart c
...
while ($row = $result->fetch_assoc()) {
  $items_array[] = [
    'product_id' => $row['id'],  // <-- Ganti 'id' jadi 'product_id'
    'name' => $row['name'],
    'portion' => 'Regular',
...
// Masukin ke tabel orders
$items_json = json_encode($items_array);
$stmt = $conn->prepare("
    INSERT INTO orders (user_id, items, total_price, timestamp) 
    VALUES (?, ?, ?, NOW())
");
$stmt->bind_param("isd", $user_id, $items_json, $cart_total);
$stmt->execute();
...
<head>
  ...
  <title>Pesanan Berhasil - SwiftMeal</title>
  ...
  <link rel="stylesheet" href="assets/css/checkout_success.css">
</head>
  ...
  <main>
    <div class="checkout-success-container">
      <div class="checkout-success-card">
        <h1>Terima kasih!</h1>
        <p>Pesananmu telah berhasil dibuat.</p>
        <a href="shop.php" class="back-to-menu-btn">Kembali ke Shop</a>
      </div>
    </div>
  </main>
